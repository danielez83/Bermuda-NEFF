...
%% Import data
load('../Matlab data/BATSandNOAAdata.mat')
load('../Matlab data/MeteoDataSYNC_GridCellSST_1x1_HF.mat')
load('../Matlab data/BermudaMeteoDataSYNC_Local_HF.mat')
load('../Matlab data/OBSdates_20210512.mat') % Sampling dates
...
subplot(2,1,1)
    ...
    for j = 1 : length(ObsDates)
        line([ObsDates(j) ObsDates(j)], ylimits, 'Color', [.7 .7 .7], 'LineWidth', .5, 'LineStyle', ':')
        hold on
    end
    ...
    plot(BermudaWS_DataSYNC.TimeVector, BermudaWS_DataSYNC.goodSST, 'Color', StGeorgeCOL, 'LineWidth', 2)
    scatter(BATS.Time, BATS.SST_C, 'Marker', BATSoceanSYM, 'MarkerEdgeColor', 'k', 'MarkerFaceColor',BATSoceanCOL)
    % OSTIA data
...
subplot(2,1,2)
    ...
    scatter(BATS.Time, BATS.SAL_psu, 'Marker', BATSoceanSYM, 'MarkerEdgeColor', 'k', 'MarkerFaceColor',BATSoceanCOL)
    ylabel('Salinity (PSU)')
    xlim([datetime('01/05/2013','InputFormat','dd/MM/uuuu'); ...
        datetime('01/02/2014','InputFormat','dd/MM/uuuu')]);
    ylim([35.4 37])
...
TTStGeorge = table2timetable(BermudaWS_DataSYNC);
...
h3b = plot(TTStGeorge.TimeVector, TTStGeorge.goodSST - TTOSTIA.SST, 'Color', StGeorgeCOL, 'LineWidth', 2); h3b.Color(4)=0.10;  % 75% opaque
scatter(TTStGeorge.TimeVector(goodIndexes), TTStGeorge.goodSST(goodIndexes) - TTOSTIA.SST(goodIndexes), 'MarkerEdgeColor', 'k', 'MarkerFaceColor',StGeorgeCOL)
...
EqStGeorged18O = d18_V(SeaWater_d18O, TTStGeorge.goodSST(goodIndexes)+273.15);
...
EqStGeorgedD = d2_V(SeaWater_dD, TTStGeorge.goodSST(goodIndexes)+273.15);
